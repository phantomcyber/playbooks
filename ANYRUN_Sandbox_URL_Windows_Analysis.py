"""
This playbook is designed for automated URL analysis in the ANY.RUN Sandbox on a Windows virtual machine. It enriches artifacts through a ready-made workflow: submitting objects to the sandbox, retrieving verdicts, extracting IoCs, and generating detailed reports on malicious objects.
"""
...
def on_start(container):
    ...
    # call 'detonate_url_windows' block
    detonate_url_windows(container=container)

    return

@phantom.playbook_block()
def detonate_url_windows(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, loop_state_json=None, **kwargs):
    phantom.debug("detonate_url_windows() called")
    ...
    # build parameters list for 'detonate_url_windows' call
...
def get_analysis_verdict(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, loop_state_json=None, **kwargs):
    ...
    detonate_url_windows_result_data = phantom.collect2(container=container, datapath=["detonate_url_windows:action_result.data.*.analysis_id","detonate_url_windows:action_result.parameter.context.artifact_id"], action_results=results)

    parameters = []

    # build parameters list for 'get_analysis_verdict' call
    for detonate_url_windows_result_item in detonate_url_windows_result_data:
        if detonate_url_windows_result_item[0] is not None:
            parameters.append({
                "analysis_id": detonate_url_windows_result_item[0],
                "context": {'artifact_id': detonate_url_windows_result_item[1]},
            })

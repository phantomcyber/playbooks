"""
This playbook is designed for automated file analysis in the ANY.RUN Sandbox on a Windows virtual machine. It enriches artifacts through a ready-made workflow: submitting objects to the sandbox, retrieving verdicts, extracting IoCs, and generating detailed reports on malicious objects.
"""
...
def on_start(container):
    ...
    # call 'detonate_file_windows' block
    detonate_file_windows(container=container)
...
def get_analysis_verdict(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, loop_state_json=None, **kwargs):
    ...
    detonate_file_windows_result_data = phantom.collect2(container=container, datapath=["detonate_file_windows:action_result.data.*.analysis_id","detonate_file_windows:action_result.parameter.context.artifact_id"], action_results=results)

    parameters = []

    # build parameters list for 'get_analysis_verdict' call
    for detonate_file_windows_result_item in detonate_file_windows_result_data:
        if detonate_file_windows_result_item[0] is not None:
            parameters.append({
                "analysis_id": detonate_file_windows_result_item[0],
                "context": {'artifact_id': detonate_file_windows_result_item[1]},
            })
...
@phantom.playbook_block()
def detonate_file_windows(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, loop_state_json=None, **kwargs):
    phantom.debug("detonate_file_windows() called")
    ...
    # build parameters list for 'detonate_file_windows' call
    ...
    phantom.act("detonate file windows", parameters=parameters, name="detonate_file_windows", assets=["anyrun"], callback=get_analysis_verdict)

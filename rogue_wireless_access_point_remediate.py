...
"""
def collect_data(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None):
    ...
    check_whitelist(container=container)
...
"""
Check the results of the WiFi scan against our whitelist of official company networks which are saved as a Custom List called "Example Company WiFi ESSID Whitelist". Stop investigating any networks that are both on the whitelist and are using WPA2.

In this case we are not pursuing further investigation of WPA2 networks that match the whitelist because our client devices will not automatically connect to them.
"""

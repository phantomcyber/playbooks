"""
This playbook runs lastline actions one by one.
...
def detonate_file_cb(action, success, incident, results, handle):
    ...
    phantom.act('detonate url', parameters=[{ "url" : "www.phantomcyber.com" }], assets=["lastline"], callback=detonate_url_cb)
...
def get_process_file_cb(action, success, incident, results, handle):
    ...
    phantom.act('detonate file', parameters=parameters, assets=["lastline"], callback=detonate_file_cb)

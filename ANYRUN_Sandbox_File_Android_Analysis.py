"""
This playbook is designed for automated file analysis in the ANY.RUN Sandbox on an Android virtual machine. It enriches artifacts through a ready-made workflow: submitting objects to the sandbox, retrieving verdicts, extracting IoCs, and generating detailed reports on malicious objects.
"""
...
def on_start(container):
    ...
    # call 'detonate_file_android' block
    detonate_file_android(container=container)
...
def get_analysis_verdict(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, loop_state_json=None, **kwargs):
    ...
    ################################################################################
    # Retrieves the verdict based on the results of analyzing the file by analysis_id
    ################################################################################

    detonate_file_android_result_data = phantom.collect2(container=container, datapath=["detonate_file_android:action_result.data.*.analysis_id","detonate_file_android:action_result.parameter.context.artifact_id"], action_results=results)

    parameters = []

    # build parameters list for 'get_analysis_verdict' call
    for detonate_file_android_result_item in detonate_file_android_result_data:
        if detonate_file_android_result_item[0] is not None:
            parameters.append({
                "analysis_id": detonate_file_android_result_item[0],
                "context": {'artifact_id': detonate_file_android_result_item[1]},
            })
    ...
    phantom.act("get analysis verdict", parameters=parameters, name="get_analysis_verdict", assets=["anyrun"], callback=decision_1)

    return


@phantom.playbook_block()
def detonate_file_android(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, loop_state_json=None, **kwargs):
    phantom.debug("detonate_file_android() called")
    ...
    # build parameters list for 'detonate_file_android' call
    ...
    phantom.act("detonate file android", parameters=parameters, name="detonate_file_android", assets=["anyrun"], callback=get_analysis_verdict)
...
def get_iocs(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, loop_state_json=None, **kwargs):
    ...
    ################################################################################
    # Retrieves the CSV Report with IoCs extracted from file analysis by analysis_id
    ################################################################################
...
def get_report(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, loop_state_json=None, **kwargs):
    ...
    # Suspicious file by analysis_id
    ################################################################################
...
def build_output_malicious(action=None, success=None, container=None, results=None, handle=None, filtered_artifacts=None, filtered_results=None, custom_function=None, loop_state_json=None, **kwargs):
    ...
    get_report_result_data = phantom.collect2(container=container, datapath=["get_report:action_result.data.*.analysis_url","get_report:action_result.data.*.tags"], action_results=results)
    ...
    get_report_result_item_1 = [item[1] for item in get_report_result_data]
    ...
    phantom.save_block_result(key="build_output_malicious__inputs:0:get_report:action_result.data.*.analysis_url", value=json.dumps(get_report_result_item_0))
    ...
    phantom.save_block_result(key="build_output_malicious__inputs:4:get_report:action_result.data.*.tags", value=json.dumps(get_report_result_item_1))

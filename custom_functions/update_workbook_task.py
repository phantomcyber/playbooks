def update_workbook_task(task_name=None, note_title=None, note_content=None, status=None, owner=None, container_id=None, **kwargs):
    ...
    Args:
        ...
        note_title (CEF type: *): Note title goes here. Enter 0 to treat as empty
        note_content (CEF type: *): Body of note goes here. Enter 0 to treat as empty
        status (CEF type: *): One of: incomplete, in progress, complete. Enter 0 to treat as empty.
        owner (CEF type: *): Owner to assign task to. Enter 0 to treat as empty
        container_id (CEF type: phantom container id): ID of Phantom Container
    ...
    if task_name != 0:
        ...
        if task_count > 1:
            ...
            if task_is_note_required and note_content == 0 and status == 'complete' and len(task_notes) == 0:
                ...
                if note_content != 0:
                    if note_title != 0:
                        phantom.debug(phantom.add_note(container=container_id, note_type='task', task_id=task_id, title=note_title, content=note_content))
                    else:
                        phantom.debug(phantom.add_note(container=container_id, note_type='task', task_id=task_id, content=note_content)[1]) 
                elif note_content == 0:
